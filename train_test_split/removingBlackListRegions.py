...
parser.add_argument('--tenMB_GC', action='store', nargs=1, type=str, required = True, help='file from orderGCcontent.py with sorted avg 10MB GC content')
parser.add_argument('--blacklist', action='store', nargs=1, type=str, required = True, help='bed file with blacklist regions or reference gene Loci')
args=parser.parse_args()
tenMB_GC = args.tenMB_GC[0]
blacklist = args.blacklist[0]

#use bedtools intersect
bedtools_intersect_out = subprocess.check_output("bedtools intersect -a {} -b {} -wa -wb".format(tenMB_GC, blacklist), shell=True).decode("utf-8").splitlines()
logging.info('intersection complete')
logging.info(datetime.datetime.now())
bedtools_dont_intersect_out = subprocess.check_output("bedtools intersect -a {} -b {} -v".format(tenMB_GC, blacklist), shell=True).decode("utf-8").splitlines()
...
windowsDict = {}
for i, value in enumerate(bedtools_intersect_out): #chr blacklist_start blacklist_end chr window_start window_end gc
    #fileToWriteTo2.write(value + '\n')
    fields = value.split("\t")
    chr = fields[0]
    gc = fields[3]

    blacklist_start = int(fields[5])
    blacklist_end = int(fields[6])+1
    window_start = int(fields[1])
    window_end = int(fields[2])+1

    valuesOI = (chr, window_start, window_end, gc)
...
for key in windowsDict:
    chr, window_start, window_end, gc = key
    window_end += -1
    ...
    for value in leaps:
        start, end = value
        fileToWriteTo.write(chr + '\t' + str(start) + '\t' + str(end) + '\t' + str(gc) + '\n')
