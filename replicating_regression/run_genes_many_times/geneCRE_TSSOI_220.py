...
class regress_gene_cre():
    ...
    def run(self, chrom, thresh_type, lessone, iterations, output_file_name):
        '''
        ...
        for k_val in range(iterations):
            new_output_file_name = 'iter_{}_{}'.format(k_val, output_file_name)
            r = self.runRefine(rt.copy(), np.copy(ss), self.pair)
            ...
            rt['ma'] = r['ma']
            rt['pair'] = self.pair
            with open("{}.{}.{}.{}.pickle".format(new_output_file_name, self.chrom, self.thresh_type, self.lessone), "wb") as f:
                pickle.dump(rt, f, protocol=pickle.HIGHEST_PROTOCOL)
            self.run_extract_info([self.chrom], [self.thresh_type], [self.lessone], new_output_file_name, [809, 1017, 1393])
        return
        #return rt
    ...
    def run_extract_info(self, chrom_list, ttype_list, lessone_list, output_file_name, utN_s):
        ...
        for chrom in self.chrom_list:
            ...
            for g in ttype_list: #this is thresh_type
                #****#*********#*************#***********#
                used_tt = tt_dict[g]
                #****#*********#*************#***********#
                for c in lessone_list: #this is lessone cell type
                    ...
                    tt = []
                    for utN in utN_s:
                        tt_n = np.where(self.pair['TSSidx']==self.ut[utN])[0]
                        for value in tt_n:
                            tt.append(value)
                    tt = np.array(tt)
                    info_all = np.zeros((rt['pair']['TSS'][tt].shape[0]), dtype=np.dtype([('TSS', np.int32), ('CRE', np.int32), #convert pair into a numpy array
                                                            ('TSSidx', np.int32), ('rr', np.float32), ('sel', np.int32)]))

                    info_all['TSS'] = rt['pair']['TSS'][tt]
                    info_all['CRE'] = rt['pair']['CRE'][tt]
                    info_all['TSSidx'] = rt['pair']['TSSidx'][tt]
                    info_all['rr'] = rt['pair']['rr'][tt]
                    info_all['sel'] = rt['sel'][tt]
...
thresh_type = 2
i = 0
test1.run(chrom, thresh_type, i, iterations, output_file_name)
#with open("{}.{}.{}.{}.pickle".format(output_file_name, chrom, thresh_type, i), "wb") as f:
#    pickle.dump(rt, f, protocol=pickle.HIGHEST_PROTOCOL)

#test1.run_extract_info(chrom_list, new_output_file_name)
