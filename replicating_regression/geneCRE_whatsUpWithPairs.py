...
class regress_gene_cre():
    ...
    def lm(self, x,y, intercept=False):
        ...
        else: #replace with scipy equivalents?
    ...
    def run(self, chrom, thresh_type, lessone):
        ...
        if self.e is None:
            ...
            self.e = self.lm(sss, self.rna.ravel(order='F'))['coeff'] #Use QR decomposition to solve linear regression; e is a vector of 27 coefficients
        ...
        #make a state mask array; horizontal stacking; 2d array
        ...
        print('After genereg PAIR SHAPE: ', self.pair.shape)
        print("Select CREs..." , flush=True)
        r = self.runRefine(rt, ss, self.pair)
        print('After runRefine PAIR SHAPE: ', self.pair.shape)
        ...
        rt['ma'] = r['ma']
        rt['pair'] = self.pair
        print('BEFORE PICKLE PAIR SHAPE: ', rt['pair'].shape)
        return rt
