...
parser = ap.ArgumentParser(description='Replicating Regression')
...
class subselection_regression():
    def __init__(self, expression_file, TSS_file, TSS_intersect_file, cCRE_file, within_1kb_file, within_1Mb_file, cellTypesOI):
        self.genes_Y_file = expression_file
        self.TSS = TSS_file
        self.TSS_intersect = TSS_intersect_file
        self.cCRE_file = cCRE_file
        self.within_1kb = within_1kb_file
        self.within_1Mb = within_1Mb_file

        self.cellTypesOI = cellTypesOI
        self.num_cellTypesOI = len(cellTypesOI)

        self.num_states = len(range(1,27))

        self.gene_index = {}
        self.num_genes = 0
        self.populate_gene_index()

        self.cellTypesOI_index = {}
        self.Y_exp_matrix = np.zeros((self.num_cellTypesOI, self.num_genes), dtype = np.float64)
        self.populate_cellType_index_and_expression_matrix()

        self.num_ccRE = 205019
        self.ccREs_state_prop = np.zeros((self.num_ccRE,self.num_cellTypesOI,self.num_states+1))
        self.ccRE_index = {}
        self.populate_ccRE_index_and_state_prop()

    '''Assign an index to correspond to a gene ID'''
    def populate_gene_index(self):
        genes = set()
        with open(self.genes_Y_file) as f:
            for line in f:
                fields = line.strip('\r\n').split('\t')
                genes.add(fields[3])
        ...
        self.gene_index = dict(zip(genes, range(self.num_genes)))

    '''prepare a matrix mapping a gene ID to a vector of expression by cell type'''
    def populate_cellType_index_and_expression_matrix(self):
        with open(self.genes_Y_file) as f:
            for j, line in enumerate(f):
                ...
                for i, minifield in enumerate(expression_fields):
                    ...
                    if j == 0:
                        self.cellTypesOI_index[cellType] = i
                    self.Y_exp_matrix[self.cellTypesOI_index[cellType], self.gene_index[gene_ID]] = np.log2(float(expression)+0.001)

    '''assign an index to correspond to each ccRE
    Also make an array that stores the proportion of the ccRE that is in each state in each cell type'''
    def populate_ccRE_index_and_state_prop(self):
        with open (self.cCRE_file) as f:
            for i, line in enumerate(f):
                fields = line.strip('\r\n').split('\t')
                self.ccRE_index[fields[0]] = i
    ...
    def encode_genomeloc_to_ID(self,chr, start, end):
        chrEncode = {'chr1':'A',
                    'chr2':'B',
                    'chr3':'C',
                    'chr4':'D',
    ...
    def initial_eRP_calc():
        TSS_intersect_dict = {}
        with open(self.TSS_intersect) as f:
            for line in f:
                fields = line.strip('\r\n').split('\t')
        ...
        initial_props = np.zeros((self.num_cellTypesOI, self.num_genes, self.num_states),dtype=np.float64)
        for key in self.gene_index:
            ...
            if key in TSS_intersect_dict:
                ...
                for i in range(num_intersect):
                    ...
                    ccRE_ID = self.encode_genomeloc_to_ID(chr, ccRE_start, ccRE_stop)
                    states_minifields = states.split(";")[:-1]
                    for k, element in enumerate(states_minifields):
                        cellType, state = element.split("=")
                        if state != "0" and cellType in self.cellTypesOI:
                            prop_overlap = self.get_overlap([TSS_start-100, TSS_start+100], [ccRE_start, ccRE_stop])
                            initial_props[self.cellTypesOI_index[cellType], self.gene_index[key], int(state)-1] += prop_overlap #state-1 because state 0 not in this array, therefore state 1 in index 0, etc....

        initial_regression_coeffs = np.ndarray((self.num_cellTypesOI, self.num_states+1))
        initial_regression_coeffs[:,0] = 0 #state 0 has coefficient 0

        '''linear regression'''
        for cellType in self.cellTypesOI:
            print(cellType)
            indice = self.cellTypesOI_index[cellType]

            fit_init_model = linear_model.LinearRegression().fit(np.array(initial_props[indice]),np.array(self.Y_exp_matrix[indice]))
        ...
        self.initial_regression_coeffs = initial_regression_coeffs
        ...
        self.initial_eRP = np.sum(self.ccREs_state_prop*self.initial_regression_coeffs.reshape((1,self.num_cellTypesOI,self.num_states+1)),axis=2) #shape should be (num_ccRE, num_cellTypesOI)

sub_reg = subselection_regression(genes_Y_file, TSS, TSS_intersect, cCRE_file, within_1kb, within_1Mb, cellTypesOI)
