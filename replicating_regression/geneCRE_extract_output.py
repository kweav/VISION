...
class regress_gene_cre():
    ...
    def extract_gene_ccRE(self, info_all, rna_g): #axis=1
        gene_tss = np.matrix(rna_g[info_all[2], np.arange(0,4)])
        gene_ccRE = (info_all[1]-1)*200
        info_pos = np.concatenate((gene_tss, gene_ccRE, info_all[2], info_all[3], info_all[4]))
        return info_pos
    ...
    def run_extract_info(self, chrom_list, output_file_name):
        ...
        for chrom in self.chrom_list:
            ...
            #used_id_1 = np.array((m <= self.m_thresh) & (s <= self.s_thresh))
            #used_id_2 = np.array((m <= self.m_thresh) & (s > self.s_thresh))
            #used_id_3 = np.array((m > self.m_thresh) & (s > self.s_thresh))
            #used_id_4 = np.array((m > self.m_thresh) & (s <= self.s_thresh))
            #used_id_mat = np.hstack((used_id_1, used_id_2, used_id_3, used_id_4))
            print("chr: ", chrom)
            for g in range(1,5): #this is thresh_type
                #used_id_g = used_id_mat[:,g-1]
                #****#*********#*************#***********#
                used_tt = tt_dict[g]
                #****#*********#*************#***********#
