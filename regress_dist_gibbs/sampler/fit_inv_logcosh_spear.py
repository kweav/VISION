...
'''changes from stacked_X_inv_logcosh_spear.py:
1) instead of priors for beta, let's just fit
'''
...
def main():
    ...
    for chrom in chrom_list:
        ...
        #model.set_up_prior_info(args.sigma_gamma_alpha, args.sigma_gamma_beta, args.gamma_norm_mu, args.gamma_norm_var, args.Sigma_invwishart_v_0, args.Sigma_invwishart_S_0, args.theta_MVN_Lambda_0, args.theta_MVN_mu_0)
        model.set_up_prior_info(args.sigma_gamma_alpha, args.sigma_gamma_beta, args.gamma_norm_mu, args.gamma_norm_var)
...
def generate_parser():
    ...
    parser.add_argument('--k_beta_a', action='store', dest='k_beta_a', type=int, default=3)
    parser.add_argument('--k_beta_b', action='store', dest='k_beta_b', type=int, default=1)
...
class regress_sampler():
    ...
    def no_pair_justP(self, tss_i):
        ...
        if np.sum(np.isnan(y_hat)) > 0:
            print("no_pair_justP NaN {}".format(tss_i), flush=True)
        return y_hat
    ...
    def pair_noP(self, tss_i, CREs_within, indicator_boolean):
        ...
        sum = np.sum(subset_adjusted, axis=0)
        if np.sum(np.isnan(sum)) > 0:
            print('pair_noP NaN {}'.format(tss_i), flush=True)
        return sum
    ...
    def compute_spearmanr(self, tss_i, cre_weighted_sum): #note we are not adjusting for distance when looking at this correlation
    ...
    def indicator_function(self, tss_i, CREs_within_bool):
        beta_e = np.hstack(([0],self.stacked_beta[self.stateN-1:])).reshape((1,-1))
        weighted_sum = self.find_weighted_sum(beta_e, CREs_within_bool)
        #CREs_within_adjusted = self.adjust_by_distance(weighted_sum, self.TSSs[tss_i], self.cre_coords[CREs_within_bool, 0], self.cre_coords[CREs_within_bool, 1])
        return np.abs(self.compute_spearmanr(tss_i, weighted_sum)) >= self.k
    ...
    def regression_equation(self, tss_i):
        ...
        if np.sum(CREs_within) == 0: #no pairing possible
            ...
            PairingFlag = False
            self.build_X_e[tss_i] = np.zeros((self.cellN, self.stateN-1))
        ...
        if np.sum(indicator_boolean) == 0: #no pairing happened
            ...
            self.build_X_e[tss_i] = np.zeros((self.cellN, self.stateN-1))
            return self.no_pair_justP(tss_i), PairingFlag
        yhat = self.no_pair_justP(tss_i) + self.pair_noP(tss_i, CREs_within, indicator_boolean)

        return yhat, PairingFlag

    def find_MSE(self, i, yhat):
        try:
            MSE = metrics.mean_squared_error(self.exp_values[i], yhat)
        #logging.info('begin find_MSE(): ' + str(datetime.datetime.now()))
        except ValueError:
            nans = np.sum(np.isnan(yhat))
            infs = np.sum(np.isinf(yhat))
            print("NaNs: " ,nans, "\tInfs: ", infs, "\tTSS{}".format(i), flush=True)
            MSE = 10**9
        return MSE
    ...
    def run_regression_equation(self, initialTime=False):
        ...
        if initialTime:
            self.build_X_e = np.zeros((self.tssN, self.cellN, self.stateN-1), dtype=np.float32)
        ...
        for i in range(self.tssN):
            ...
            totalMSE += self.take_logcosh(self.exp_values[i], yhat)
    ...
    def take_logcosh_loss(self, true, yhat):
        '''keras logcosh = log((exp(x) + exp(-x))/2)'''
        sess = tf.InteractiveSession()
        logcosh_array = losses.logcosh(true.astype(np.float32), yhat.astype(np.float32)).eval()
        sess.close()
        loss = np.sum(logcosh_array)
        return loss

    def take_logcosh(self, true, yhat):
        sess = tf.InteractiveSession()
        logcosh = losses.logcosh(np.float32(true), np.float32(yhat)).eval()
        sess.close()
        return logcosh

    def posterior_gamma(self, sigma_sqr, yhat_values):
        #logging.info('begin posterior_gamma(): ' + str(datetime.datetime.now()))
        generated_u = stats.uniform.rvs()
        logcosh_value = self.take_logcosh_loss(self.exp_values, yhat_values)
        sampled = 0.7 + np.sqrt((-20/3)*(np.log(generated_u/2*np.sqrt(3/5*np.pi))+(1/(2*sigma_sqr)*logcosh_value)))
        #logging.info('end posterior_gamma(): ' + str(datetime.datetime.now()))
        return sampled

    def posterior_k(self, sigma_sqr, yhat_values):
        #logging.info('begin posterior_k(): ' + str(datetime.datetime.now()))
        generated_u = stats.uniform.rvs()
        logcosh_value = self.take_logcosh_loss(self.exp_values, yhat_values)
        sampled = np.sqrt(generated_u/3 * np.exp(1/(2*sigma_sqr)*logcosh_value))
    ...
    #     lambda_0_inv = self.take_inv(self.theta_MVN_Lambda_0)
    #     Sigma_inv = self.take_inv(Sigma)
    ...
    #     Lambda_n = self.take_inv(lambda_0_inv + self.stacked_beta_n*Sigma_inv)
    ...
    #     Sigma_inv = self.take_inv(Sigma)
    #     variance = self.take_inv(Sigma_inv + (stacked_X_data.T).dot(stacked_X_data)/sigma_sqr)
    ...
    def posterior_sigma_sqr(self, yhat_values):
        #logging.info('begin posterior_sigma_sqr(): ' + str(datetime.datetime.now()))
        generated_u = stats.uniform.rvs()
        logcosh_val = self.take_logcosh_loss(self.exp_values, yhat_values)
        precision = -1 * np.log(generated_u)/(0.5*logcosh_val + 1)
    ...
    #def set_up_prior_info(self, sigma_gamma_alpha, sigma_gamma_beta, gamma_norm_mu, gamma_norm_var, Sigma_invwishart_v_0, Sigma_invwishart_S_0, theta_MVN_Lambda_0, theta_MVN_mu_0):
    def set_up_prior_info(self, sigma_gamma_alpha, sigma_gamma_beta, gamma_norm_mu, gamma_norm_var):
    ...
    def report_metrics(self, iteration, MSE_sum, numNP, minNotPaired):
        ...
        toWriteTo.write('Iteration:\t{}\tlogcosh_sum:\t{}\tnotPaired:\t{}\tnotPairedRatio1:\t{}\tnotPairedRatio2:\t{}\tyhat_nans:\t{}\n'.format(iteration, MSE_sum, numNP, (minNotPaired - numNP)/self.tssN, numNP/self.tssN, self.yhat_nan))
        toWriteTo.close()
    ...
    def run_sampler(self, init_beta, init_gamma, init_k, init_sigma_sqr, iters, burn_in, cre_dist):
        ...
        self.report_metrics(-1, minMSE, minNotPaired, minNotPaired)
