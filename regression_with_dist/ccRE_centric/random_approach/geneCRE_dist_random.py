...
def main():
    ...
    for chrom in chrom_list:
        ...
        model.build_pairing_array(args.iterations, args.psubset)
        model.build_metric_arrays()
        model.set_initial_betas()
        model.find_initial_weighted_sum()
        model.drive_pairing(args.dist_gamma, args.cre_dist, args.correlation)
...
def generate_parser():
    ...
    parser.add_arugment('--p', '--psubset', action='store', dest='psubset', type=int, default=200)
...
class regress_gene_cre():
    ...
    def build_pairing_array(self, iterations, psubset):
        self.iter = iterations
        self.psubset = psubset
        self.pairing_array = np.full((self.tssN, self.creM, (self.iter*self.psubset)+1, self.cellN), -1, dtype=np.int32) #4D array of tssN * creM * (iter*psubset)+1 * cellN to store pairings for initial pairing and refinement iterations

    def build_metric_arrays(self):
        self.mse_array = np.empty((self.tssN, (self.iter*self.psubset)+1, self.cellN), dtype=np.float32) #3D array of tssN * (iter*psubset)+1 * cellN to store MSE for initial pairings and refined pairings

    def find_yhat(self, linear_model, X):
        X = X.reshape(-1, self.stateN)[:,1:]
        return linear_model.predict(X)

    def find_MSE(self, i, iter_val, psub): #if calling for initial pairing psub=-1, iter_val=0.
        cre_inclusions = self.creIndex_range[self.pairing_array[i,:,psub+(self.psubset*iter_val)+1, self.lessone] == 1]
        ...
        self.mse_array[i, (iter_val*psub)+1, self.lessone] = metrics.mean_squared_error(Y, yhat)
    ...
    def get_dim_of_pairing(self, tss_i, iter_val, psub, lessone): #if calling for initial_pairing psub=-1, iter_val=0
        return np.sum(self.pairing_array[tss_i, :, psub+(self.psubset*iter_val)+1, lessone] == 1) #will return number of CREs paired in that slice
    ...
    def refine_pairs(self, tss_i, iter_val, psub):
        cre_inclusions = self.creIndex_range[self.pairing_array[tss_i,:,psub+(self.psubset*iter_val)+1, self.lessone] == 1]
        inclusions_to_test = powerset_of_inclusions = self.powerset(cre_inclusions)
        if powerset_of_inclusions.shape[0] > self.psubset:
            lens_obs = [len(s) for s in powerset_of_inclusions]
            centered_lengths = np.abs(lens_obs - np.mean(lens_obs))
            from_parabola = np.square(centered_lengths)
            prob = np.full(generated_powerset.shape[0], 1, dtype=np.float64)
            prob /= from_parabola
            prob /= np.sum(prob)
            inclusions_to_test = np.random.choice(powerset_of_inclusions, self.psubset, p=prob)
        for n in range(self.psubset):
            self.pairing_array[tss_i,inclusions_to_test[n],psub+(self.psubset*iter_val)+1, self.lessone] = 1
            self.find_MSE(tss_i, iter_val, psub)
    ...
    def drive_pairing(self, dist_gamma, lessone, cre_dist, correlation):
        ...
        for i in range(self.tssN):
            ...
            if self.get_dim_of_pairing(i, 0, -1, 0) == 0: #notate no pairings
